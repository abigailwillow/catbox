...
exports.getUser = (userid, callback) => {
    ...
    if (cachedUser) {
        ...
        let query = `SELECT userid, balance, streak, activity
            FROM Users
            WHERE userid = '${userid}'`
        db.get(query, (err, row) => {
            if (err) {
                ...
                if (typeof callback === 'function') {
                    callback(row ? new exports.User(row.userid, row.balance, row.streak, row.activity) : undefined)
                } else {
...
exports.getRichestUsers = (max = 10, callback) => {
    ...
    db.all(query, (err, rows) => {
        if (err) {
            ...
            rows.forEach(row => {   
                row ? users.push(new exports.User(row.userid, row.balance, row.streak, row.activity)) : undefined
            })
