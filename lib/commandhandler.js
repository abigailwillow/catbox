...
function command (name, description, admin, arguments, fn) {
    ...
    this.run = function (msg, args) {
        switch (this.admin) {
            case 1:
                if (!msg.member.roles.find(x => x.hasPermission('ADMINISTRATOR'))) { throw new Error(txt.err_no_admin) }
                break;
            case 2:
                if (!cfg.operators.includes(msg.author.id)) { throw new Error(txt.err_no_operator) }
                break;
        }
        ...
        for (i = 0; i < this.arguments.length; i++) {
                ...
                switch (argument) {
                    case 'member':
                        ...
                        if (identifier === '*') {
                            ...
                            if (value === null) {
                                throw new Error(txt.err_no_user)
                            }
                        }
                        break;
                    case 'channel':
                        ...
                        if (value === null) {
                            throw new Error(txt.err_no_channel)
                        }
                        break;
                    case 'string':
                        value = args[i]
            ...
            if (!isValid) throw new Error(txt.err_invalid_arg + argument)
