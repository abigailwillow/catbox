...
function command (name, description, admin, args, fn) {
    this.name = name
    ...
    this.run = function (msg, args) {
        switch (this.admin) {
            case 1:
                if (!msg.member.permissions.has('Administrator')) { throw txt.err_no_admin }
                break;
            case 2:
                ...
                switch (argument) {
                    case 'member':
                        ...
                        if (identifier === '*') {
                            ...
                            value = msg.guild.members.cache.find(x => 
                                x.id === identifier ||
                    ...
                    case 'channel':
                        ...
                        value = msg.guild.channels.cache.find(x => x.type === 0 && (x.id === identifier || x.name.toLowerCase().includes(identifier)))
                        if (value === null || value === undefined) {
