...
const jsonFile = './data/backups/backups.json'
    ...
    if (cachedUser) {
        ...
        db.get(query, (err, row) => {
            if (err) {
                ...
                if (typeof callback === 'function') {
                    callback(row ? new this.User(row.userid, row.balance, row.streak) : undefined)
                } else {
...
exports.User = class User {
    ...
    set id(value) {
        if (this.id == null) {
    		...
    		throw `Attempted to set user ${this.id}'s ID to ${value}. Value is read-only!`          
        }
    }
...
cron.schedule('* * * * *', () => {
    ...
    file.copyFile('./data/data.db', `./data/backups/data-${dateFormat}.db`, err => { if (err) { throw err } })
    file.access(jsonFile, file.constants.F_OK, (err) => {
        if (err) {
            console.error(err)
            file.writeFile(jsonFile, JSON.stringify([{filename: `data-${dateFormat}.db`, timestamp: date.getTime()}], null, 4), err => { if (err) { throw err } })
        } else {
            file.readFile(jsonFile, (err, data) => { 
                if (err) {
                    throw err
                } else {
                    data = JSON.parse(data)
                    data.push({
                        filename: `data-${dateFormat}.db`,
                        timestamp: date.getTime()
                    })
                    file.writeFile(jsonFile, JSON.stringify(data, null, 4), err => { if (err) { throw err } })
                }
            })
        }
    })
