        ...
        this.users.forEach(user => {
            ...
            db.run(query, err => {
                if (err) {
                    ...
                    if (typeof callback === 'function' && index === this.users.size) {
                        callback()
                    }
                }
            })
...
exports.User = class User {
    ...
    changeBalance(amount, ignoreCheck = false) {
        let allowed = this.balance + amount >= 0 || ignoreCheck
...
cron.schedule('0 * * * *', () => {
    this.updateUsers(() => {
        ...
        this.users.forEach(user => {
            ...
            totalCats += user.activity
            totalUsers += user.activity > 0 ? 1 : 0
            user.activity = 0
        })
        
        console.log(`(${date.toISOString().substr(11, 8)}) Database backup was made and ${totalCats} hourly cats were spawned for ${totalUsers} users.`)
    })
})